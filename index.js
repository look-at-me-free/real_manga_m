(() => {
  ...
  const CONFIG = {
    ...
    compactTraversalRadius: 5,
    compactTraversalEdgeThreshold: 11
  };
  ...
  function isInteractiveTarget(target) {
    ...
    return !!target.closest('input, textarea, select, button, a, [contenteditable="true"], [contenteditable=""], .search-input');
  }
  ...
  function createCompactEntryWindow(entries, currentIndex, radius = CONFIG.compactTraversalRadius) {
    const total = entries.length;
    if (!total) return [];

    if (total <= CONFIG.compactTraversalEdgeThreshold) {
    ...
    if (start > 0) {
      result.push({ type: "gap", key: "left-gap" });
    }

    for (let i = start; i <= end; i++) {
      result.push({ type: "entry", entry: entries[i], index: i });
    }

    if (end < total - 1) {
      result.push({ type: "gap", key: "right-gap" });
    }

    return result;
  }

  function buildCompactTraversalBar(entries, currentIndex, prev, next, opts = {}) {
    const {
      position = "top",
      includeSearch = false,
      currentButtonBehavior = "scroll-bottom"
    } = opts;
    ...
    bar.appendChild(
      makeTraversalPill(
        "← Previous",
        prev ? () => switchEntry(STATE.currentWork.slug, prev.slug, false, {
          actionSource: position === "top" ? "prev" : "prev-bottom"
        }) : null,
        "",
        !prev
      )
    ...
    const currentAction = () => {
      if (currentButtonBehavior === "scroll-top") {
        const target = document.getElementById("readerTopAnchor") || document.getElementById("reader");
        if (target) target.scrollIntoView({ behavior: "smooth", block: "start" });
        return;
      }
    ...
    bar.appendChild(
      makeTraversalPill(currentLabel, currentAction, "current jump-full")
    );
    ...
    for (const item of windowed) {
      if (item.type === "gap") {
        bar.appendChild(createEl("span", "traversal-gap", "…"));
        continue;
      }
      ...
      bar.appendChild(
        makeTraversalPill(
          label,
          () => switchEntry(STATE.currentWork.slug, item.entry.slug, false, {
            actionSource: position === "top" ? "chapter-pill-top" : "chapter-pill-bottom"
          })
        )
      );
    }
    ...
    bar.appendChild(
      makeTraversalPill(
        ...
        next ? () => switchEntry(STATE.currentWork.slug, next.slug, false, {
          actionSource: position === "top" ? "next" : "next-bottom"
        }) : null,
        "",
        !next
      )
  ...
  function buildDesktopTopTraversal(entries, currentIndex, prev, next) {
    ...
    shell.appendChild(
      buildCompactTraversalBar(entries, currentIndex, prev, next, {
        position: "top",
        currentButtonBehavior: "scroll-bottom"
      })
    );
  ...
  function buildBottomTraversal(entries, currentIndex, prev, next) {
    ...
    if (!STATE.isMobileReader) {
      ...
      shell.appendChild(
        buildCompactTraversalBar(entries, currentIndex, prev, next, {
          position: "bottom",
          currentButtonBehavior: "scroll-top"
        })
      );
  ...
  function wireProgressWatch() {
    ...
    const onScroll = () => {
      ...
      window.requestAnimationFrame(() => {
        ...
        let progress = scrollable > 0 ? scrollTop / scrollable : 0;

        if (window.innerHeight + scrollTop >= doc.scrollHeight - 2) {
          progress = 1;
        }
  ...
  function wireKeyboardNavigation() {
    if (STATE.keyboardNavWired) return;
    STATE.keyboardNavWired = true;
    ...
    document.addEventListener("keydown", async (e) => {
      ...
      if (e.key === "ArrowLeft" && prev) {
        e.preventDefault();
        burstServeAds();
        await switchEntry(STATE.currentWork.slug, prev.slug, false, { actionSource: "keyboard-prev" });
        return;
      }
      ...
      if (e.key === "ArrowRight" && next) {
        e.preventDefault();
        burstServeAds();
        await switchEntry(STATE.currentWork.slug, next.slug, false, { actionSource: "keyboard-next" });
      }
    });
  }
